...
const ProductCard = ({ product }) => {
  ...
  const brandLink = product.brandSlug ? `/brands/${product.brandSlug}` : '/contact';
    ...
    <div className="group relative flex h-full flex-col overflow-hidden rounded-[1.8rem] border border-white/70 bg-white/80 shadow-[0_24px_70px_-42px_rgba(107,44,145,0.8)] backdrop-blur transition duration-500 hover:-translate-y-2">
      ...
      <div className="relative aspect-[4/4.1] overflow-hidden px-8 pb-4 pt-16">
        ...
        <div className="absolute left-6 top-6 z-10">
          <span className="rounded-full bg-[#351343] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-white shadow-lg">
            {product.brand}
          </span>
        </div>
      ...
      <div className="flex flex-1 flex-col p-6">
        <h3 className="font-serif text-2xl text-[#2f1538]">{product.name}</h3>
        ...
        <div className="mt-6 flex items-center justify-between border-t border-[#f3e4ec] pt-5">
          <span className="text-sm font-semibold uppercase tracking-[0.28em] text-[#7a2c8e]">{product.brand}</span>
          <Link to={brandLink} className="text-sm font-semibold uppercase tracking-[0.18em] text-[#9e2b86] transition hover:text-[#351343]">
            {actionLabel}
          </Link>
        </div>
      </div>
